...
class Grid extends GameObject
{
    ...
    constructor(holder, x, y, width, height)
    {
        ...
        this.tileSize = screen.width; // Tamanho dos quadros.
    ...
    Start()
    {
        ...
        console.log(this.maxTile.x + ", " + this.maxTile.y);
    }
            ...
            for(var i = 0; i < possition.length; i++)
            {
                if(this.grid[possition[i].x] != undefined)
                {
                    ...
                    if(this.grid[possition[i].x][possition[i].y] != undefined)
                    {
                        ...
                        // Eleva consideravelmente o tempo.
                        ...
                        if(add)
                        {
                            for(var j = 0; j < this.ocuppedPositions.length; j++)
                            {
                                if(this.ocuppedPositions[j].x == possition[i].x && this.ocuppedPositions[j].y == possition[i].y)
                                {
                                    break;
                                }
                            }
    ...
    GetObjectPossition(object)
    {
        ...
        {
            var minX = Math.floor(((object.transform.possition.x - object.collider[i].transform.width/2) - this.transform.possition.x) / this.tileSize) - zone;
            var maxX = Math.floor(((object.transform.possition.x + object.collider[i].transform.width/2) - this.transform.possition.x) / this.tileSize) + zone;
            var minY = Math.floor(((object.transform.possition.y - object.collider[i].transform.height/2) - this.transform.possition.y) / this.tileSize) - zone;
            var maxY = Math.floor(((object.transform.possition.y + object.collider[i].transform.height/2) - this.transform.possition.y) / this.tileSize) + zone;
